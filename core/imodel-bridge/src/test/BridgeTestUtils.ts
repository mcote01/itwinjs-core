...
import { CodeSpecs, RectangleWidget, SmallSquareWidget } from "./integration/TestBridgeElements";
...
export class BridgeTestUtils {
  ...
  private static initDebugLogLevels(reset?: boolean) {
    ...
    Logger.setLevel(BentleyLoggerCategory.Performance, reset ? LogLevel.Error : LogLevel.Warning);
  ...
  public static verifyIModel(imodel: IModelDb, bridgeJobDef: BridgeJobDefArgs, isUpdate: boolean = false) {
    ...
    assert.equal(6, getCount(imodel, "TestBridge:EquilateralTriangleWidget"));
    assert.equal(8, getCount(imodel, "TestBridge:IsoscelesTriangleWidget"));
    assert.equal(isUpdate ? 7 : 8, getCount(imodel, "TestBridge:LargeSquareWidget"));
    assert.equal(isUpdate ? 2 : 1, getCount(imodel, "TestBridge:RectangleWidget"));
    assert.equal(10, getCount(imodel, "TestBridge:RightTriangleWidget"));
    assert.equal(8, getCount(imodel, "TestBridge:SmallSquareWidget"));
    ...
    if (!isUpdate) {
      const ids = ExternalSourceAspect.findBySource(imodel, rlinkid!, "Widget", "e1aa3ec3-0c2e-4328-89d0-08e1b4d446c8");
      assert.isTrue(Id64.isValidId64(ids.aspectId!));
      assert.isTrue(Id64.isValidId64(ids.elementId!));
      const widget = imodel.elements.getElement<SmallSquareWidget>(ids.elementId!);
      assert.equal(widget.condition, "New");
    } else {
      // Modified element
      let ids = ExternalSourceAspect.findBySource(imodel, rlinkid!, "Widget", "e1aa3ec3-0c2e-4328-89d0-08e1b4d446c8");
      assert.isTrue(Id64.isValidId64(ids.aspectId!));
      assert.isTrue(Id64.isValidId64(ids.elementId!));
      let widget = imodel.elements.getElement<SmallSquareWidget>(ids.elementId!);
      assert.equal(widget.condition, "Scratched");

      // New element
      ids = ExternalSourceAspect.findBySource(imodel, rlinkid!, "Widget", "5b51a06f-4026-4d0d-9674-d8428b118e9a");
      assert.isTrue(Id64.isValidId64(ids.aspectId!));
      assert.isTrue(Id64.isValidId64(ids.elementId!));
      widget = imodel.elements.getElement<RectangleWidget>(ids.elementId!);
      assert.equal(widget.placement.origin.x, 1.0);
      assert.equal(widget.placement.origin.y, 2.0);
    }
  }
}
